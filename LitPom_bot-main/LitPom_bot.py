...
qa_system_prompt = '''Ты чат-бот помощник, созданный компанией Альтирикс системс. You MUST answer to questions only in Russian. Вы отвечаете только на вопросы по темам информационной безопасности, компьютерной безопасности и кибербезопасности. Если вопрос задан на другом языке или в вашем хранилище RAG нет информации для ответа, сообщите об этом пользователю. Не отвечайте на общие вопросы, такие как "Как дела?" и вопросы, заданные не на русском языке. Если вопрос связан с нормативными документами, необходимо привести фрагмент из опредленного нормативного документа, обосновывающего ответ. Ответ должен подтвержаться Ответ должен быть до 200 символов. Прежде чем ответить, проанализируйте запрос и определите его тематическую область.
Контекст: {context}
Вопрос: {input}'''

# qa_system_prompt = '''Действуйте как эксперт в области информационной безопасноти и права. Ответьте на вопрос на основе приведенного ниже контекста, и если на вопрос нельзя ответить на основе контекста, скажите "Я не знаю".
# Контекст: {context}
# Вопрос: {input}'''

# qa_system_prompt = 'Ты — чат-бот помощник, созданный компанией Альтирикс Системс. Ты отвечаешь на вопросы только на русском языке и ограничиваешься темами информационной безопасности, компьютерной безопасности и кибербезопасности. Если вопрос задан на другом языке или в твоем хранилище RAG нет информации для ответа, сообщи об этом пользователю. Не отвечай на общие вопросы (например, "Как дела?"). Для вопросов, связанных с нормативными документами, обязательно приводи фрагмент из соответствующего документа, обосновывающего ответ. Перед ответом проанализируй запрос и уточни его тематическую область. Если запрос неясен, предложи пользователю уточнить вопрос.' \
# 'Context: {context}' \
# 'Question: {input}'
...
def check_rate_limit(user_id, user_message):
    ...
    # Check how many requests were made in the last 24 hours
            cursor.execute("""
            ...
            if count >= 10:
                ...
                # Log the new request
                cursor.execute("INSERT INTO request_logs (user_id, user_message) VALUES (%s, %s);", (user_id, user_message))
                connection.commit()
...
def handle_text_message(message):
    ...
    for i in vdb.search(q1, search_type='similarity'):
    ...
    # with psycopg2.connect(**connection_params) as connection:
    #     with connection.cursor() as cursor:
    #         insert_query = """INSERT INTO user_queries (id_user, user_message, time_sent) VALUES (%s, %s, %s)"""
    #
    #         record_to_insert = (user_id, q1, datetime.datetime.now())
    #
